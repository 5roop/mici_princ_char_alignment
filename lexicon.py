...
def write_fst_with_silence(
    ...
    print("{state}\t{final_cost}".format(state=loop_state, final_cost=0.0), file=file)


def prepare_lexicon(
    lexicon, silence_phones, nonsilence_phones, optional_silence, oov, dir: Path
):
    ...
    for w, p, trans in lexicon:
        for ph in trans:
            assert (
                ph in nonsilence_phones or ph in silence_phones
            ), f"ERROR: {ph} is not a proper phoneme!"
    ...
    prefix = (
        set()
    )  # set of all possible prefixes (does not include full transcriptions)
    for w, p, trans in lexicon:
        t = trans.copy()
        while len(t) > 0:
            t.pop()
    ...
    with open(dir / "phones.txt", "w") as f:
        for p, i in phone_map.items():
            f.write(f"{p} {i}\n")
            isyms.add_symbol(p, i)
    print(f'Wrote {dir / "phones.txt"}...')

    with open(dir / "disambig.int", "w") as f:
        for i in range(max_disambig + 1):
            d = phone_map[f"#{i}"]
            f.write(f"{d}\n")
    print(f'Wrote {dir / "disambig.int"}...')
    ...
    print(f'Wrote {dir / "words.txt"}...')

    with open(dir / "word_boundary.int", "w") as f:
        cnt = 1
        ...
        for i in range(len(nonsilence_phones)):
            for b in ["begin", "end", "singleton", "internal"]:
                f.write(f"{cnt} {b}\n")
                cnt += 1
    print(f'Wrote {dir / "word_boundary.int"}...')
...
nonsilence_phones = sorted([*"abcčdefghijklmnoprsštuvzžčćšđ"])
...
silence_phones = sorted(["sil", "sp", "spn"])
optional_silence = "sp"
oov = "<unk>"
graphemes2np = {
    "lj": "L",
...
def apply_transformations_to_list(input_string, transformations):
    import re

    # Sort keys by length, longest first to handle overlapping keys
    sorted_keys = sorted(transformations, key=len, reverse=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("wlist", type=Path)
    ...
    words = []
    with open(args.wlist) as f:
        for l in f:
            words.append(l.strip())

    words_to_lexicon(words, args.model, args.outdir)
